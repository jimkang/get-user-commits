...
function GetCommitsForRepos({
  ...
  function getCommitsForRepos(
    ...
    function postNextQuery() {
      var query = getCommitQuery(reposThatHaveCommitsToGet, userEmail);
      if (queryLogger) {
    ...
    function handleCommitResponse(res, body, done) {
      if (!body) {
        passNonFatalError(
          new Error('Empty body received from commit request.')
        );
      } else if (body.errors) {
  ...
  for (var queryId in viewer) {
    ...
    if (queryResult) {
      if (queryResult.defaultBranchRef) {
        ...
        let commits = pluck(
          queryResult.defaultBranchRef.target.history.edges,
          'node'
        );
